...
import { MARK_EMOJI, resultLine, shareText, type Result } from "@/lib/found/result";
      ...
      {result && result.marks.length > 0 && (
        <p className={styles.result}>
          <span className={styles.marks} aria-hidden="true">
            {result.marks.map((m) => MARK_EMOJI[m]).join("")}
          </span>
          <span>{resultLine(result).replace("I got", "You got")}</span>
        </p>
      )}
          ...
          <div className={styles.send}>
            ...
            <div className={styles.row}>
              <a
                ...
                onClick={() => play.verdict("share:whatsapp")}
              >
                Send on WhatsApp
              </a>
              {canShare && (
